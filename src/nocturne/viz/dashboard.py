...
// control channel: lets an embedding deck drive the demo (scrub nights, focus a panel)
window.addEventListener("message",e=>{try{const m=e.data||{};if(m.nocturne&&m.night)render(Math.max(1,Math.min(N,+m.night)));
  if(m.nocturne&&m.focus){const el=$(m.focus);if(el)el.scrollIntoView({behavior:"smooth",block:"center"});}}catch(_){}});
</script></body></html>
"""
