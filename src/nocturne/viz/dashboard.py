...
body::before{content:"";position:fixed;inset:0;z-index:-2;background:
  radial-gradient(900px 500px at 80% -5%, rgba(217,178,90,.10), transparent 60%),
  linear-gradient(180deg, rgba(6,7,15,.74), rgba(4,5,11,.92));}
...
function drawConstellation(d){const svg=$("constellation"),W=580,H=380,cx=W/2,cy=H/2;
  const groups=nodesByThread(d.memory),keys=Object.keys(groups);
  let s=`<defs><radialGradient id="sky" cx="50%" cy="44%" r="60%"><stop offset="0%" stop-color="rgba(40,46,92,.5)"/><stop offset="100%" stop-color="rgba(6,7,15,0)"/></radialGradient>
  ...
  const cents=keys.map((k,i)=>{const a=(i/keys.length)*6.283-1.5708;return{x:cx+Math.cos(a)*128,y:cy+Math.sin(a)*98};});
  for(let i=0;i<cents.length;i++){const a=cents[i],b=cents[(i+1)%cents.length];
    s+=`<line x1="${a.x.toFixed(1)}" y1="${a.y.toFixed(1)}" x2="${b.x.toFixed(1)}" y2="${b.y.toFixed(1)}" stroke="#d9b25a" stroke-opacity=".10"/>`;
    s+=`<line x1="${a.x.toFixed(1)}" y1="${a.y.toFixed(1)}" x2="${cx}" y2="${cy}" stroke="#d9b25a" stroke-opacity=".08"/>`;}
  keys.forEach((k,gi)=>{const gx=cents[gi].x,gy=cents[gi].y,g=groups[k];
    g.items.forEach((it,ii)=>{const a2=(ii/Math.max(1,g.items.length))*6.283,r=20+g.items.length*3.2;
      const x=gx+Math.cos(a2)*r,y=gy+Math.sin(a2)*r,rad=3.5+Math.min(11,it.weight*4.2);
      const op=(0.35+Math.min(0.65,it.weight*0.45)).toFixed(2); // forgetting: faint stars are fading
      const recon=(k==="meeting" && RECON && cur>=RECON && /wednesday|11:00/.test(it.content.toLowerCase()));
      s+=`<line x1="${gx.toFixed(1)}" y1="${gy.toFixed(1)}" x2="${x.toFixed(1)}" y2="${y.toFixed(1)}" stroke="${g.th.c}" stroke-opacity=".25"/>`;
      s+=`<circle class="node" data-c="${esc(it.content)}" data-p="${esc((it.provenance||[]).join(', ')||'—')}" data-w="${it.weight.toFixed(2)}" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${rad.toFixed(1)}" fill="${g.th.c}" fill-opacity="${op}" filter="url(#glow)" style="cursor:pointer">${recon?'<animate attributeName="r" values="'+rad.toFixed(1)+';'+(rad+3).toFixed(1)+';'+rad.toFixed(1)+'" dur="1.6s" repeatCount="indefinite"/>':''}</circle>`;});
    s+=`<text x="${gx.toFixed(1)}" y="${(gy-16-g.items.length*3.2).toFixed(1)}" fill="${g.th.c}" font-family="PlexMono,monospace" font-size="11" text-anchor="middle">${k}</text>`;});
  if(!keys.length)s+=`<text x="${cx}" y="${cy}" fill="#76728f" font-family="PlexMono" font-size="13" text-anchor="middle">memory empty</text>`;
  svg.innerHTML=s;
  ...
  svg.querySelectorAll(".node").forEach(n=>n.onclick=()=>{$("inspector").innerHTML=`<b>★</b> ${n.dataset.c} <span style="color:var(--faint)">(w=${n.dataset.w})</span><br><span style="color:var(--faint)">why:</span> from raw items [${n.dataset.p}]`;});}

function drawTwoBrains(t,d){
  // ours: the constellation (few glowing nodes)
  const W=460,H=240,cx=W/2,cy=H/2;let so=`<rect width="${W}" height="${H}" rx="12" fill="rgba(8,9,18,.4)"/>`;
  const mem=d.memory.slice().sort((a,b)=>b.weight-a.weight).slice(0,14);
  mem.forEach((it,i)=>{const a=(i/Math.max(1,mem.length))*6.283,r=40+(i%3)*22;const x=cx+Math.cos(a)*r*1.5,y=cy+Math.sin(a)*r;
  ...
  $("b-ours").innerHTML=so;$("ours-sub").textContent=`${d.facts} stars · one figure you can hold in your head`;
  // catalog: dense field scaled by accumulation facts
  const lib=DATA.brains.gbrain_accumulate, libN=lib?lib.facts[t-1]:0, dots=Math.min(libN,520);
  let sl=`<rect width="${W}" height="${H}" rx="12" fill="rgba(8,9,18,.4)"/>`;let s=12345+t;const r=()=>{s=(s*9301+49297)%233280;return s/233280;};
  for(let i=0;i<dots;i++)sl+=`<circle cx="${(r()*W).toFixed(1)}" cy="${(r()*H).toFixed(1)}" r="${(r()*1.3+.5).toFixed(2)}" fill="#7fa8ff" fill-opacity="${(.3+r()*.5).toFixed(2)}"/>`;
  $("b-lib").innerHTML=sl;$("lib-sub").textContent=`${libN} pages · every star kept, none connected`;}

function drawMemory(d){const items=d.memory.slice().sort((a,b)=>b.weight-a.weight);
  $("mem").innerHTML=items.length?items.map(it=>`<li onclick="document.getElementById('inspector').innerHTML='<b>★</b> '+this.dataset.c+' <span style=color:var(--faint)>why: ['+this.dataset.p+']</span>'" data-c="${esc(it.content)}" data-p="${esc((it.provenance||[]).join(', ')||'—')}"><span class="w">${it.weight.toFixed(2)}</span><span class="tier ${it.tier}">${it.tier.replace("_"," ")}</span><span>${esc(it.content)}</span></li>`).join(""):`<li style="color:var(--faint)">empty</li>`;}
function drawBriefing(d){$("briefing").innerHTML=d.briefing?md(d.briefing):"<span style='color:var(--faint)'>no briefing</span>";}
function drawLedger(d){if(!d.ledger||!d.ledger.length){$("ledger").innerHTML="<span style='color:var(--faint)'>no hypotheses yet</span>";return;}
  ...
  $("ledger").innerHTML=a.map(h=>`<div class="hyp ${h.status}"><div>${h.status==="confirmed"?"✦ ":""}${esc(h.statement)}</div><div class="meta">${h.status} · conf ${h.confidence.toFixed(2)}${h.confirmed_night?` · confirmed night ${h.confirmed_night}`:""}</div></div>`).join("");}
function drawPolicy(d,t){if(!d.policy||!d.policy.length){$("policy").innerHTML="<span style='color:var(--faint)'>seed policy</span>";return;}
  $("policy").innerHTML=d.policy.slice().sort((a,b)=>b.weight-a.weight).map(p=>`<div class="pol ${p.learned_night===t?'fresh':''}"><div>${esc(p.content)}</div><div class="ln">w=${p.weight.toFixed(2)} · learned night ${p.learned_night}${p.learned_night===t?' · NEW':''}</div></div>`).join("");}
...
$("play").onclick=function(){if(timer){clearInterval(timer);timer=null;this.textContent="▶ play";return;}this.textContent="⏸ pause";timer=setInterval(()=>{if(cur>=N){clearInterval(timer);timer=null;$("play").textContent="▶ play";return;}render(cur+1);},700);};
